...
Run `./scripts/verify.sh` (if available) or `make status` to check your environment. Required:
...
| `save-devnet.sh` | Save named DEVNET snapshot |
| `restore-devnet.sh` | Restore from named DEVNET snapshot |
| `run-l3-l4-tests.sh` | Run L3/L4 automated test scenarios |
...
| `EVM_DEV_MNEMONIC` | Mnemonic for Anvil accounts |
...
```bash
# Run full verification to see all issues:
./scripts/verify.sh
```
