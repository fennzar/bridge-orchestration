...
### HIGH-9 — Cross-token double-credit: claim-row token is mutable per `zephTxId`  ⚠️  (INV-2)
- **Where:** `packages/bridge/src/claims/ingest.ts:203` (un-claimed path `token: token`, vs claimed path `existing?.token ?? token` at `:137`); `packages/bridge/src/claims/signer.ts:78` (re-sign consumption check uses the mutable `rec.token`); claim row keyed by `Claim.id = bytes32(zephTxId)`.
- **What (found in review, 2026-06-10):** on-chain `usedZephyrTx` is *per token contract*, so cross-token at-most-once relies entirely on the off-chain claim row's `token` being immutable per deposit. It is not: while a claim is un-claimed, re-ingesting the same `zephTxId` under a different supported asset recomputes `token` from the *current* asset. If the token mutates A→B and the admin re-sign path (`resignClaim`) runs, it checks `usedZephyrTx(B)` (false — only A was minted) and signs a second voucher → mint on both A and B for one deposit.
- **Reachability:** narrow — needs one `zephTxId` to be seen as two assets (a multi-asset V2 tx, or a watcher asset-confusion) **and** the operator-gated `resignClaim` to fire. Single-asset deposits never mutate, so no direct attacker exploit today. But the guarantee is an *unenforced assumption*, not a control.
- **Root question (needs a decision, not a one-liner):** the claim row is keyed by `zephTxId` alone, yet Zephyr V2 txs can carry multiple asset outputs. Either (a) **enforce one-asset-per-deposit** and freeze `token` immutably per txid (`ingest.ts:203` → `existing?.token ?? token`, + reject/log a differing-asset re-ingest), or (b) **key the claim/consumption by `(zephTxId, asset)`** if multi-asset deposits are legitimate (freezing would silently drop a second-asset credit). Resolve against the Zephyr deposit model (zephyr-reference) before promoting INV-2. Naive freeze risks dropping legitimate credits under interpretation (b).
- **Status:** documented; INV-2 stays **PARTIAL**. No code change made (protocol-model decision pending owner/peer).
