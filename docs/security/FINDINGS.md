...
### HIGH-9 — Cross-token double-credit: claim-row token was mutable per `zephTxId`  ✅  (INV-2) — FIXED 2026-06-10
- **Where:** `packages/bridge/src/claims/ingest.ts` (the un-claimed `base` path recomputed `token` from the *current* transfer asset, vs the claimed paths which already froze `existing?.token ?? token`); `packages/bridge/src/claims/signer.ts:78` (re-sign consumption check keys off `rec.token`); claim row keyed by `Claim.id = bytes32(zephTxId)`.
- **What (found in review, 2026-06-10):** on-chain `usedZephyrTx` is *per token contract*, so cross-token at-most-once relied entirely on the off-chain claim row's `token` being immutable per deposit. It was not: while a claim was un-claimed, re-ingesting the same `zephTxId` under a different supported asset recomputed `token` from the *current* asset. If the token mutated A→B and the admin re-sign path (`resignClaim`) ran, it checked `usedZephyrTx(B)` (false — only A was minted) and signed a second voucher → mint on both A and B for one deposit.
- **Protocol model (decided):** a Zephyr deposit `transfer` is single-asset-typed — the watcher reads one `asset_type` per `get_transfers` entry (`watcher-zephyr/index.ts:193`) and the claim is keyed by `bytes32(txid)`, so the design already represents **one token per txid**. A same-txid/different-asset report is therefore anomalous (wallet/RPC confusion), never protocol-normal. So interpretation (a) is correct and the freeze has **zero normal-case behavior change** — it does not drop a legitimate second-asset credit, because the txid-keyed row could never hold two tokens regardless.
- **Fix:** the claim-row token is now **immutable per `zephTxId`** — frozen to the first-seen token. New pure helper `resolveClaimToken(existingToken, currentToken)` (`claims/token.ts`) returns the existing token verbatim and flags a mismatch; `freezeClaimToken()` in `ingest.ts` wraps it, `console.warn`s the anomaly, and is applied at **all three** token-write sites. `resignClaim` now always checks/signs the frozen token → a second different token can never be minted for one deposit (re-traced & confirmed in review, 2026-06-10; no other `ClaimRecord.token` write sites exist). Pinned by node:test `[INV-2] LB-CLAIM-TOKEN` (HIGH-9 case asserts a differing current token never mutates the frozen token).
- **Out of scope (noted, NOT a double-credit path):** the un-claimed path still recomputes `to` (from the stable depositor→EVM account mapping, not attacker-controlled per-report) and `amountWei` (the real on-chain amount) from the current report. On-chain `usedZephyrTx` bounds mint-once regardless of these, so neither is an INV-2 cross-token double-credit; they are INV-1 economic-field-immutability considerations, deliberately left unchanged to avoid an early-mempool-amount-freeze regression.
- **Status:** ✅ FIXED on `fix/crit1-unwrap-amount-binding`. INV-2 → **HELD**.
