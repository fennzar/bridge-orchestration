...
# ===========================================
# Frozen Test Setup — Bridge Infrastructure
# ===========================================
# Frozen copy of dev-setup.sh for test infrastructure. This script provides a
# stable, repeatable setup that tests can depend on regardless of how dev-setup.sh
# evolves over time.
#
# DO NOT refactor to call dev-setup.sh. This must remain self-contained and stable
# even as dev-setup.sh changes (different pool sizes, new contracts, etc.).
#
# Key difference from dev-setup.sh:
...
# Cleanup handler: only stop on FAILURE, leave running on success
cleanup() {
    local exit_code=$?
    if [ $exit_code -ne 0 ]; then
        echo ""
        log_info "Setup FAILED — stopping apps + infrastructure..."
        ...
        log_error "Test setup failed — everything stopped"
    else
        log_success "Test setup complete — stack left running for tests"
    fi
    exit $exit_code
}
trap cleanup EXIT
...
echo "==========================================="
echo "  Test Setup — Bridge Infrastructure"
echo "  (frozen — independent of dev-setup.sh)"
echo "==========================================="
echo ""
...
# Restart Anvil (tests need it running)
$DC_DEV start anvil
log_info "Waiting for Anvil..."
for i in $(seq 1 20); do
    cast block-number --rpc-url http://127.0.0.1:8545 >/dev/null 2>&1 && break
...
# ===========================================
# Step 14: Write marker + done
# ===========================================
touch "$ORCH_DIR/config/.test-setup-done"

echo ""
echo "==========================================="
log_success "Test setup complete (stack left running)"
